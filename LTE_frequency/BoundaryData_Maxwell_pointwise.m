function [coeff,fb_ini_1d] = BoundaryData_Maxwell_pointwise(N,N_omega,index,alpha1,alpha2,alpha3,fb_ini)
...
term3 = ones(N,1)*Wp'; term3 = kron(eye(N_omega),term3);
term4 = kron(ones(N_omega,1),Xp) * ones(1,length(H_pos(1,:)));
term4 = term4 .* H_pos;
term3 = term3*term4;
term5 = Wp' * Xp;
term3 = Hpp' * diag(Weight_pp.*X_pp) * term3/term5;

% term3 = X_mm*ones(1,length(H_neg(1,:)));
% term3 = Equi_p*Weight_mu_m*(term3.*H_neg);
% term3 = Hpp' * diag(Weight_pp.*X_pp) * term3;
% term4 = Wp' * Xp/2;
% term3 = term3/term4;
